...
def make_sentiment(value):
    ...
    "*** YOUR CODE HERE ***"
    return value  #uh... this is the only way I can think of to implement this.  But it seems kind of stupid...
...
def sentiment_value(s):
    ...
    "*** YOUR CODE HERE ***"
    return s  #?  This is really really weird    
...
def analyze_tweet_sentiment(tweet):
    ...
    arr = tweet_words(tweet)
    total = 0
    count = 0
    for x in arr:
        sentiment = get_word_sentiment(x)
        if has_sentiment(sentiment):
            total = total + sentiment_value(sentiment)
            count += 1
    if(total==0):
        return average
    av = total/len(arr)
    average = make_sentiment(av)
    return average
